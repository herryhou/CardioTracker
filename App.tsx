...
import { Plus, FileSpreadsheet, History as HistoryIcon, LayoutDashboard, Settings } from 'lucide-react';
...
export default function App() {
  ...
  const handleSave = (record: BPRecord) => {
      ...
      <header className="bg-white border-b border-slate-100 px-6 py-5 sticky top-0 z-40 bg-white/80 backdrop-blur-md">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <button 
            ...
            title="Export CSV to Google Sheets"
          >
      ...
      <main className="max-w-md mx-auto px-4 py-6">
        {activeTab === 'dashboard' ? (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="mt-8">
                <h3 className="font-semibold text-slate-700 mb-3 px-1">Recent History</h3>
                <div className="space-y-3">
                    ...
                    {records.length === 0 && (
                         <div className="text-center py-8 text-slate-400 text-sm">No recent records.</div>
                    )}
                </div>
            </div>
          </div>
          ...
          <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
             <h2 className="text-xl font-bold text-slate-800 mb-4 px-1">All Records</h2>
            {records.map(record => (
